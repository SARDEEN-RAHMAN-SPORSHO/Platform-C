    ...
    <script>
        ...
        async function loadVideo() {
            ...
            try {
                ...
                // TEST: Check if proxy URL is accessible
                try {
                    const testRes = await fetch(proxyUrl, { 
                        method: 'HEAD',
                        headers: { 'Range': 'bytes=0-0' }
                    });
                    console.log('Proxy test status:', testRes.status);
                    console.log('Proxy test headers:', Object.fromEntries(testRes.headers));
                    if (testRes.status !== 200 && testRes.status !== 206) {
                        const errorText = await fetch(proxyUrl).then(r => r.text());
                        console.error('Proxy error response:', errorText);
                        throw new Error('Proxy returned error: ' + testRes.status + ' - ' + errorText);
                    }
                } catch (testError) {
